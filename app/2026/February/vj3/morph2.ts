...
export function vj3Morph2(scene: THREE.Scene) {
  ...
  const init = () => {
    for (let i = 0; i < loopNum; i++) {
      ...
      mesh.position.x = (i / loopNum) * 2 - 1;
      // mesh.rotateY(i / loopNum);
      mesh.rotateZ((i / loopNum) * 2);
      lines.push(mesh);
      group.add(mesh);
    }
    scene.add(group);
  ...
  const update = (props: any) => {
    ...
    lines.forEach((line, i) => {
      ...
      // mat.lineWidth += (0.03 - mat.lineWidth) * 0.9;
