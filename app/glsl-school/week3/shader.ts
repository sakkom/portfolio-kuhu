...
void main() {
  ...
  for (float i = 0.0; i < loopNum; i++) {
    ...
    vec2 move = lemniscate(uAngle + position, uRms);
    float minRadius = rand1(i + 1.0) * 0.1;
    float shape;
  ...
  vec3 color = hsl2rgb(vec3(metaball + uAngle, 1.0, 0.5)) * brightness;
  vec3 metaballColor = metaball * color;

  vec3 outSide = (1.0 - metaball) * hsl2rgb(vec3(uAngle, 1.0, 0.2));
